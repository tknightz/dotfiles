...
##
set -g FISH_AI_KEYMAP_1 ctrl-space
...
function _fish_ai_install --on-event fish_ai_install
    ...
    echo "🍬 Installing dependencies. This may take a few seconds..." "$install_dir/bin/pip" -qq install "$(_fish_ai_get_installation_url)"
    if test $status -ne 0
